...
describe('Gallery Routes', function() {
  beforeEach(done => { //make a user and save it to db, needed for GET, PUT, POST, DELETE
    new User(testUser).generatePasswordHash(testUser.password)
    .then(user => {
      user.save();
      this.tempUser = user;
  ...
  afterEach( () => delete testGallery.userID); //remove user ID
  ...
  describe('POST: /api/gallery', function() {
    ...
    describe('wthout a valid token', function() {
  ...
  describe('GET: /ap1/s0m3rand0mwr0ngr0ut3', function() {
  ...
  describe('GET: /api/gallery/:id', function() {
    ...
    describe('wthout a valid totken', function() {
      it('should return a 401 error', done => {
        request.get(`${url}/api/gallery/${this.tempGallery._id}`)
        .end(err => {
          expect(err.status).to.equal(401);
  ...
  describe('PUT: /api/gallery/:id', function() {
    ...
    describe('without a valid gallery id', function() {
      it('should return a 404 error', done => {
